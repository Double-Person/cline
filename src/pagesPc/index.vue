  ...
  <div class="contain">
    <div class="header">
      <div class="header-contain">
        <div class="header-left">
          <img :src="'./static/imgPc/logo.png'" alt class="logo" @click="linkTo(5)" />
          <span>LSIBSS支撑系统欢迎您,{{userInfo.nickName}}</span>
        </div>
        <div class="header-right">
          <div style="margin: 0 20px 0 0;cursor:pointer" @click="linkTo(6)" v-show="userInfo.chargePower">
            <span>货款管理</span>
          </div>
          <div style="margin: 0 20px 0 0;cursor:pointer" @click="linkTo(8)">
            <span>物料生产进度</span>
          </div>
          <div style="margin: 0 20px 0 0;cursor:pointer" @click="linkTo(5)" v-show="userInfo.tasksPower">
            <span>任务安排</span>
          </div>
          <div class="message-div" @click="linkTo(0)">
            <img :src="'./static/imgPc/message-icon.png'" alt="message" class="message" />
            <div class="circle-red" v-if="haveNewMessage"></div>
          </div>
          <div class="head-div" @mouseenter="usershowfuc(true)">
            <img :src="'./static/imgPc/head-icon.png'" alt="head" class="head" />
            <span style="margin-left: 8px">{{userInfo.nickName}}</span>
          </div>
          <img :src="'./static/imgPc/loginOut-icon.png'" alt class="loginout" @click="loginout" />
        </div>
    ...
    <div class="main">
      <p class="title" v-if="model==0">
        任务列表
        <span v-if="model==0" v-bind:class="query.giveOrGet==1?'active':''" style="font-size: 16px;cursor: pointer;"
          @click="giveOrGetFuc(1)">我发布的</span>
        <span v-if="model==0" style="font-size: 16px">/</span>
        <span v-if="model==0" v-bind:class="query.giveOrGet==2?'active':''" style="font-size: 16px;cursor: pointer;"
          @click="giveOrGetFuc(2)">我接取的</span>
        <span v-if="model==0" style="font-size: 16px">/</span>
        <span v-if="model==0" v-bind:class="query.giveOrGet==3?'active':''" style="font-size: 16px;cursor: pointer;"
          @click="giveOrGetFuc(3)">待接取的</span>
        <span v-if="model==0" style="font-size: 16px">/</span>
        <span v-if="model==0" v-bind:class="query.giveOrGet==4?'active':''" style="font-size: 16px;cursor: pointer;"
          @click="giveOrGetFuc(4)">被抄送的</span>
      ...
      <div class="title" v-if="model==7" style="height:75px;">
        ...
        <div class="title-searchBox" style="float:right">
          <div class="title-searchBox-left" style="float:left;">
            <img v-bind:src="'./static/imgPc/list-icon.png'" @click="listShow" />
          </div>
          <div class="title-searchBox-mid" style="float:left;">
            <input type="text" @keyup.13="searchList(searchText)" v-model="searchText" />
          </div>
          <div class="title-searchBox-right" style="float:left;">
            <img v-bind:src="'./static/imgPc/search-icon.png'" @click="searchList(searchText)" />
          </div>
      ...
      <div class="bookmark-div" v-show="bookmark.show">
        ...
        <div class="search">
          <div>
            <input type="text" v-model="query.keywords" placeholder="请输入关键字搜索" />
            <img :src="'./static/imgPc/search-icon.png'" alt class="search-icon" @click="search" />
          </div>

          <img :src="'./static/imgPc/add-task-icon.png'" alt="添加任务" class="add-task-icon" @click="linkTo(4)" />
        </div>
      </div>
        ...
        <div class="index-right">
          <div class="condition" v-show="condition.show">
            <div class="urgent-condition">
              <img v-bind:src="query.urgent==0?'./static/imgPc/checkbox-icon.png':'./static/imgPc/checkbox-true-icon.png'"
                alt @click="clickUrgent" />
              <span>仅浏览紧急任务</span>
            </div>
            <div class="time-condition">
              ...
              <div class="block">
                ...
                <div>
                  <div v-for="(item,index) in dateArr" @click="changeData(item.id,item)" v-bind:id="item.selected?'activeDate':''"
                    class="timer" :key="index">
                    {{item.text}}

                    <!-- <img :src="'./static/imgPc/gantan.png'" alt  /> -->
                    <!-- <span v-show="item.timer.status">!</span> -->
<img :src="'./static/imgPc/gantan.png'" alt v-show="item.status" />
                  </div>
                </div>
    ...
    <div class="voice" v-show="voiceMask">
      ...
      <div>
        ...
        <el-select v-model="voiceId" placeholder="请选择" @change="voicePlay()">
          <el-option v-for="item in voiceArr" :key="item.voiceId" :label="item.name" :value="item.voiceId"></el-option>
        </el-select>
      </div>
