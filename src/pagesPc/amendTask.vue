  ...
  <div class="contain" id="addTask">
    <div class="main">
      ...
      <div>
        ...
        <div>
          <el-select v-model="form.tasksId" placeholder="请选择" @change="getDelivery">
            <el-option v-for="item in delivery" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </div>
      </div>
      <div v-show="showTypes">
        ...
        <ul class="checkBox">
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '铝材'})?checkboxTrue:checkbox" :index="'铝材'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>铝材</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '灯罩'})?checkboxTrue:checkbox" :index="'灯罩'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>灯罩</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '堵头'})?checkboxTrue:checkbox" :index="'堵头'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>堵头</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '卡扣'})?checkboxTrue:checkbox" :index="'卡扣'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>卡扣</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '弹簧'})?checkboxTrue:checkbox" :index="'弹簧'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>弹簧</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '卡簧'})?checkboxTrue:checkbox" :index="'卡簧'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>卡簧</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '吊绳'})?checkboxTrue:checkbox" :index="'吊绳'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>吊绳</span>
          </li>
          <li class="checkOption">
            <img v-bind:src="thisChecks.find(item=>{return item.name == '其他'})?checkboxTrue:checkbox" :index="'其他'" alt="选中框"
              v-model="thisChecks" @click="checkChange" />
            <span>其他</span>
          </li>
        </ul>
      ...
      <div>
        ...
        <div>
          <el-select v-model="form.types" placeholder="请选择" @change="getMid">
            <el-option v-for="item in types" :key="item.typesId" :label="item.name" :value="item.typesId"></el-option>
          </el-select>
        </div>
      </div>
      <div>
        ...
        <div>
          <el-date-picker v-model="form.dateRange" @change="dateChange" type="date" :picker-options="pickerOptions"
      ...
      <div style="height: 400px">
        ...
        <div>
          <quillEditor v-model="form.describes" :options="editorOption"></quillEditor>
        </div>
      </div>
      <div v-show="showTypes">
        <div>
      ...
      <div v-show="showTypes">
        <div>
          <span>任务环节：</span>
          <span style="font-size: 12px;color:#B0B0B0;margin-left: 16px">(可多选)</span>
        </div>
        <div class="greyBox">
        ...
        <div class="greyBox">
          ...
          <div class="carry-check">
            ...
            <div class="executive-div">
              <div class="executive-item" v-for="(item,key) in usersSelect" :key="key" @click="deleteSelect(key)">
      ...
      <div>
        ...
        <div class="greyBox">
          ...
          <div class="carry-check">
            <span>已选成员</span>
            <div class="executive-div">
              <div class="executive-item" v-for="(item,key) in usersGive" :key="key" @click="deleteGive(key)">
                <img v-bind:src="headIcon" alt="head" style="filter: blur(3px)" />
                <span class="check-span">{{item.nickName}}</span>
                <div class="check-border">
                  <span>{{key+1}}</span>
      ...
      <div>
        <div></div>
        <div>
          <button class="back" @click="submit">修改完成</button>
        </div>
      </div>
    </div>
  </div>
    ...
    mounted() {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          let newUser=res.users//接收到的用户列表
          let newexecutors=res.executors//执行人列表
          let newtask=res.links//任务环节数据列表
          let newpusher=res.pushers//推送人列表
          let newsteps=res.steps
          newUser.forEach((i)=>{
            newexecutors.forEach((a)=>{
          ...
          newUser.map((i)=>{
            if(i.isExcutor){
              this.usersSelect.push(i)
            }
            if(i.isPusher){
              this.usersGive.push(i)
            }
          })
          var arr = [];
          ...
          newsteps.map((i)=>{
            newtask.forEach((j)=>{
              if(i.stepsId==j.stepsId){
                i.selected=false,
                this.selectStep(i.stepsId)
                // console.log(this.steps,"重组后数组")
              }
            })
            //
          })
    ...
    methods: {
      ...
      submit() {
        ...
        var params = {
          ...
          endTime: this.form.endTime?this.form.endTime:this.form.dateRange, //任务截止时间
