  ...
  <div class="contain">
    ...
    <div class="main">
      <div class="main-right">
        <div class="cards">
          <div class="card-item" v-for="(item,index) in cards.list" :key="item.index" @click.stop="cardInfo(item.id)"
            v-bind:id="(item.status&&item.status==7)?'shine-box-green':''" v-bind:class="item.urgent?'shine-box-red':''">
            ...
            <div class="card-item-top">
              <div class="my_delivery">
                ...
                <div class="delivery" @click.stop="Bacmask(item.id)">{{item.shipName}}</div>
              </div>
              <div class="my-del">
                <p class="item-name" v-bind:title="item.name">{{item.name}}</p>
              ...
              <div>
                <div class="getTask" @click.stop="getTask(item.id)" v-if="item.canAccept" :id="taskIssuer==true?'cantClick':'canClick'">接受任务</div>
                <div class="taskReceive" v-else>任务接受人：{{item.acceptorName}}</div>
              </div>
            </div>
            <img :src="upindex[index]===index?'./static/imgPc/xia.png':'./static/imgPc/shang.png'" alt @click.stop="packup(index)"
              class="packup" />
            <div class="checks" v-show="upindex[index]===index">
              ...
              <ul>
                <li v-for="(items,index) in item.checks" :key="index">
                  ...
                  <img v-bind:src="items.flag=='true'?'./static/imgPc/step-true-icon.png':'./static/imgPc/default-icon.png'"
                    alt :style="item.checkStatus=='0'?'opacity:0':'opacity:1'" />
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="pagination">
          <el-pagination :page-size="30" :pager-count="11" @size-change="handleSizeChange" @current-change="handleCurrentChange"
            background layout="prev, pager, next" :total="cards.totalRow"></el-pagination>
        </div>
      </div>
    </div>
